...
class WrapperOpenAI (gym.Env):
    ...
    def user_defined_observation(self, aircraft_state_f_ks, z_dot_g_ks):
        ...
        current_theta_to_target_theta = np.abs(np.rad2deg(current_theta) - self.targetValues['targetTheta'])
        ...
        self.error_array_theta[-1] = current_theta_to_target_theta
        ...
        user_defined_observation = np.asarray(
            [current_u, current_u_to_target_u,
             current_theta, current_theta_dot, current_theta_to_target_theta])
    ...
    def reward_elevator(self, aircraft_state_f_ks, z_dot_g_ks):
        ...
        if np.rad2deg(current_theta) < self.envelopeBounds['thetaMin'] or np.rad2deg(current_theta) > self.envelopeBounds['thetaMax']:
            reward0 += -1000
        if np.abs(current_theta - self.targetValues['targetTheta']) > 1:
            reward0 += -1 * np.abs(current_theta - self.targetValues['targetTheta'])
        if np.abs(current_theta - self.targetValues['targetTheta']) <= 1:
            reward0 += 0
        return reward0
    ...
    def reward_thrust(self, aircraft_state_f_ks, z_dot_g_ks):
        ...
        if np.abs(current_u - self.targetValues['targetSpeed']) > 1:
            reward1 += -1 * np.abs(current_u - self.targetValues['targetSpeed'])
        if np.abs(current_u - self.targetValues['targetSpeed']) <= 1:
            reward1 += 0
        return reward1
