...
class WrapperOpenAI (gym.Env):
    ...
    def reset(self):
        ...
        # set targets
        self.targetValues['targetSpeed'] = np.random.uniform(42, 50)
        print('new Target: ', self.targetValues)

        # set state at initial
        phi_as_random = np.deg2rad(np.random.uniform(0, 0))
        theta_as_random = np.deg2rad(np.random.uniform(0, 0))
    ...
    def step(self, actionThrust):
        ...
        set_thrust = np.interp(actionThrust[0], [-1, 1], self.aircraft.SteuerflaechenUndMotorStellung.controlLimits['deltaThrustBorder'])
        self.aircraft.SteuerflaechenUndMotorStellung.deltaThrust = set_thrust
        # Headline: theta wird mit PID-Regler stabilisiert
        set_elevator = self.pid._innerLoopElevator(-2.5, np.rad2deg(self.aircraft.theta), self.aircraft.q,
                                                   self.aircraft.SteuerflaechenUndMotorStellung.deltaElevator)
    ...
    def user_defined_observation(self, observation):
        ...
        error_current_speed_to_target_speed = np.abs((observation[0]) - self.targetValues['targetSpeed'])

        self.observationErrorAkkumulation = np.roll(self.observationErrorAkkumulation, 1)
        self.observationErrorAkkumulation[-1] = error_current_speed_to_target_speed
        ...
        observation = np.asarray(
            [current_speed, error_current_speed_to_target_speed, self.integration_error_stepsize_, self.action_servo_command_history[0]])
    ...
    def compute_reward(self, observation):
        ...
        if (observation[0]) > self.envelopeBounds['speedMax'] or (observation[0]) < self.envelopeBounds['speedMin']:
            reward += -100
        # Abweichung abs(target-current) > 1 -> -1
        if np.abs((observation[0]) - self.targetValues['targetSpeed']) > 1:
            reward += -1
