...
class DatComGeometryC172Class(object):
    '''
    References
    ----------
    [1]
    ROSKAM, J., Methods
    for Estimating Stability and Control
        Derivatives
        of
        Conventional
        Subsonic
    ...
    Douglas
    Co., The
    USAF and Stability and Control
    Digital
    DATCOM, Users
    Manual
    '''
    ...
    def __init__(self):
        # CONVERSIONS
        self.lbs2kg = 0.453592  # Pounds (lb) to kilograms (kg)
        self.ft2m = 0.3048  # Feet (ft) to meters (m)
        self.slug2kg = 14.5939  # Slug to kilograms (kg)
        self.slugft2_2_kgm2 = 1.35581795  # Slug*feet^2 to kilograms*meters^2 (kg*m^2)

        # Mass & Inertia
        self.mass = 2300 * self.lbs2kg  # kg
        self.inertia = np.diag([948, 1346, 1967]) * self.slugft2_2_kgm2  # kg·m²
        ...
        self.Coeff_Drag_data = np.array([0.044, 0.034, 0.03, 0.03, 0.036, 0.048, 0.067, 0.093, 0.15, 0.169, 0.177, 0.184])
        ...
        self.Coeff_Lift_data = np.array([-0.571, -0.321, -0.083, 0.148, 0.392, 0.65, 0.918, 1.195, 1.659, 1.789, 1.84, 1.889])
        self.Coeff_Lift_alphadot_data = np.array([2.434, 2.362, 2.253, 2.209, 2.178, 2.149, 2.069, 1.855, 1.185, 0.8333, 0.6394, 0.4971])
        self.Coeff_Lift_q_data = np.array([7.282, 7.282, 7.282, 7.282, 7.282, 7.282, 7.282, 7.282, 7.282, 7.282, 7.282, 7.282])
        self.Coeff_Lift_delta_elev_data = np.array([-0.132, -0.123, -0.082, -0.041, 0, 0.061, 0.116, 0.124, 0.137])
        ...
        self.Coeff_PitchMoment_delta_elev_data = np.array([0.3302, 0.3065, 0.2014, 0.1007, -0.0002, -0.1511, -0.2863, -0.3109, -0.345])
        ...
        self.Coeff_YawMoment_delta_aile_data = np.array([[-0.004321, -0.002238, -0.0002783, 0.001645, 0.003699, 0.005861, 0.008099,
                                             ...
                                             0.01072, 0.01138, 0.01161, 0.01181],
                                                         [-0.002016, -0.001044, -0.000123, 0.0007675, 0.00173, 0.002735, 0.0038,
                                             0.004844, 0.00652, 0.00692, 0.00706, 0.0072],
