...
class PlotState(object):
    ...
    def addData(self, state, force, moments, alpha, beta, deltaControls, i):
        self.df = self.df.append(pd.Series([state[0], state[1], state[2], state[3], state[4], state[5], state[6],
                                            state[7], state[8], np.rad2deg(state[9]), np.rad2deg(state[10]), self.utils.normalize_angle(np.rad2deg(state[11])), force[0], force[1],
                                            force[2], moments[0], moments[1], moments[2], np.rad2deg(alpha), np.rad2deg(beta), np.rad2deg(deltaControls[0]),
                                            np.rad2deg(deltaControls[1]), np.rad2deg(deltaControls[2]), np.rad2deg(deltaControls[3]), i],
