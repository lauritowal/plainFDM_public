...
class AircraftBaever(object):
    ...
    def getState(self):
        return np.array([self.u, self.v, self.w, self.x, self.y, self.z, self.p, self.q, self.r, self.phi, self.theta, self.psi])

    def setState(self, u, v, w, x, y, z, p , q, r, phi, theta, psi):
        # ACHTUNG: Wahrscheinlich falsch... Rückgabe ist phi, theta, psi
        self.u, self.v, self.w, self.x, self.y, self.z, self.p, self.q, self.r, self.phi, self.theta, self.psi = u, v, w, x, y, z, p, q, r, phi, theta, psi
        ...
        self.staudruck = 0.5 * self.rho * np.power(self.TAS, 2)
        self.state =  np.array([u, v, w, x, y, z, p , q, r, phi, theta, psi])
